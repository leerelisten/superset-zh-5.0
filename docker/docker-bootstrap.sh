...
if [ "$DEV_MODE" == "true" ]; then
    if [ "$(whoami)" = "root" ] && command -v uv > /dev/null 2>&1; then
      # Detect if PostgreSQL is being used
      # Check DATABASE_DIALECT or SQLALCHEMY_DATABASE_URI
      NEED_POSTGRES=false
      if [[ "$DATABASE_DIALECT" == postgres* ]]; then
        NEED_POSTGRES=true
      elif [[ "$SUPERSET__SQLALCHEMY_DATABASE_URI" == postgresql* ]] || [[ "$SQLALCHEMY_DATABASE_URI" == postgresql* ]]; then
        NEED_POSTGRES=true
      fi
      
      # Install psycopg2 if PostgreSQL is detected
      if [ "$NEED_POSTGRES" == "true" ]; then
        echo "PostgreSQL detected, ensuring psycopg2 is installed"
        uv pip install psycopg2-binary || uv pip install psycopg2 || true
      fi
...
# Check both DATABASE_DIALECT and SQLALCHEMY_DATABASE_URI for PostgreSQL
NEED_POSTGRES=false
if [[ "$DATABASE_DIALECT" == postgres* ]]; then
  NEED_POSTGRES=true
elif [[ "$SUPERSET__SQLALCHEMY_DATABASE_URI" == postgresql* ]] || [[ "$SQLALCHEMY_DATABASE_URI" == postgresql* ]]; then
...
if [ "$NEED_POSTGRES" == "true" ] && [ "$(whoami)" = "root" ] && [ "$1" != "worker" ] && [ "$1" != "beat" ]; then
...
#
if [ -f "${REQUIREMENTS_LOCAL}" ] && [ "$(whoami)" = "root" ]; then
  # Check if mysqlclient is in requirements-local.txt
  if grep -q "mysqlclient" "${REQUIREMENTS_LOCAL}"; then
    echo "mysqlclient detected in requirements-local.txt, installing system dependencies..."
